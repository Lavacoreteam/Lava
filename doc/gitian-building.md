...
*Setup instructions for a Gitian build of Lava Core using a Debian VM or physical system.*
...
- [Building Lava Core](#building-bitcoin-core)
...
Get the [Debian 8.x net installer](http://bttracker.debian.org:6969/stat) (Only available as a torrent - navigate to "debian-8.5.0-amd64-netinst.iso"). a more recent minor version should also work, see also [Debian Network installation](https://www.debian.org/CD/netinst/).
...
in the Lava repository under 'Fetch and create inputs' to install sources which require
...
To build Lava Core (for Linux, OS X and Windows) just follow the steps under 'perform
...
For example:
```bash
URL=https://github.com/snguyen-vn/lava.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit lava=${COMMIT} --url bitcoin=${URL} ../bitcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit lava=${COMMIT} --url bitcoin=${URL} ../bitcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit lava=${COMMIT} --url bitcoin=${URL} ../bitcoin/contrib/gitian-descriptors/gitian-osx.yml
```
