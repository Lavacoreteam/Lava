# How to compile and execute Lava code on Windows 10

There are two possibilities of compiling Lava code on Windows 10.
 ...
 * [Xming](http://www.straightrunning.com/XmingNotes/) latest version, if you are planning to compile using [unix compiling instruction](https://github.com/lavaofficial/lava/blob/master/doc/build-unix.md)
...
4. Make shared folder between two locations
    ...
    * From Ubuntu bash window create a link to it:
            ...
            ln -s /mnt/c/Users/<username>/lava lava   
...
5. Clone git repository into newly created directory and go to the right branch
            ...
            git clone https://github.com/lavaofficial/lava.git
            cd lava
...
3. Go to depends folder and build dependencies (you may wish to build only 32-bit or 64-bit version)
        ...
        cd ~/lava
    It takes a while. You need to do it only once unless you delete lava-depends directory
...
6. Build 32-bit debug build (from the Lava root directory)
    
        ./configure --prefix=$HOME/lava-depends/i686-w64-mingw32
        make -j`nproc`

7. Build 64-bit debug build (from the Lava root directory)
     
        ./configure --prefix=$HOME/lava-depends/x86_64-w64-mingw32
        make -j`nproc`

8. After check the directory to run GUI with __lava-qt.exe__
    
        C:\Users\<username>\lava\src\qt
----
## Second method - Ubuntu + Xming
1. Use existing paper [build-unix](https://github.com/lavaofficial/lava/blob/master/doc/build-unix.md) and
...
4. From Ubuntu bash window start lava-qt:

        cd /mnt/c/Users/<username>/lava/src/qt
        ./lava-qt 
