...
//    LogPrintf("bench", "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * 0.001, nTimeReadFromDisk * 0.000001);
    ...
    if (pwalletMain->zwallet) {
        ...
        if (pblock->sigmaTxInfo->spentSerials.size() > 0) {
            ...
            pwalletMain->zwallet->GetTracker().UpdateSpendStateFromBlock(pblock->sigmaTxInfo->spentSerials);
        }

        if (pblock->sigmaTxInfo->mints.size() > 0) {
            LogPrintf("HDmint: UpdateMintStateFromBlock. [height: %d]\n", GetHeight());
            pwalletMain->zwallet->GetTracker().UpdateMintStateFromBlock(pblock->sigmaTxInfo->mints);
        }
    }
#endif
