...
UniValue elysium_getproperty(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "  \"issuer\" : \"address\",            (string) the Lava address of the issuer on record\n"
...
UniValue elysium_getorderbook(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "[                                              (array of JSON objects)\n"
            "  {\n"
            "    \"address\" : \"address\",                         (string) the Lava address of the trader\n"
...
UniValue elysium_gettradehistoryforaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            ...
            "  {\n"
            "    \"txid\" : \"hash\",                               (string) the hex-encoded hash of the transaction of the order\n"
            "    \"sendingaddress\" : \"address\",                  (string) the Lava address of the trader\n"
            "    \"ismine\" : true|false,                         (boolean) whether the order involes an address in the wallet\n"
            "    \"confirmations\" : nnnnnnnnnn,                  (number) the number of transaction confirmations\n"
            "    \"fee\" : \"n.nnnnnnnn\",                          (string) the transaction fee in lavas\n"
            "    \"blocktime\" : nnnnnnnnnn,                      (number) the timestamp of the block that contains the transaction\n"
            ...
            "        \"address\" : \"address\",                         (string) the Lava address of the other trader\n"
...
UniValue elysium_gettradehistoryforpair(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "    \"address\" : \"address\",                 (string) the Lava address of the seller\n"
            "    \"amountsold\" : \"n.nnnnnnnn\",           (string) the number of tokens sold in this trade\n"
            "    \"amountreceived\" : \"n.nnnnnnnn\",       (string) the number of tokens traded in exchange\n"
            "    \"matchingtxid\" : \"hash\",               (string) the hash of the transaction that was matched against\n"
            "    \"matchingaddress\" : \"address\"          (string) the Lava address of the other party of this trade\n"
            "  },\n"
            "  ...\n"
            "]\n"
            "\nExamples:\n"
...
UniValue elysium_getactivedexsells(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "    \"seller\" : \"address\",               (string) the Lava address of the seller\n"
            "    \"amountavailable\" : \"n.nnnnnnnn\",   (string) the number of tokens still listed for sale and currently available\n"
            "    \"lavadesired\" : \"n.nnnnnnnn\",    (string) the number of lavas desired in exchange\n"
            "    \"unitprice\" : \"n.nnnnnnnn\" ,        (string) the unit price (LAVA/token)\n"
            ...
            "      {\n"
            "        \"buyer\" : \"address\",                (string) the Lava address of the buyer\n"
            ...
            "        \"amounttopay\" : \"n.nnnnnnnn\"        (string) the amount in lavas needed finalize the trade\n"
            "      },\n"
            "      ...\n"
            "    ]\n"
            "  },\n"
    ...
    for (OfferMap::iterator it = my_offers.begin(); it != my_offers.end(); ++it) {
        ...
        int64_t sellBitcoinDesired = selloffer.getLAVADesiredOriginal(); //badly named - "Original" implies off the wire, but is amended amount
        ...
        // calculate unit price and updated amount of lava desired
        ...
        responseObj.push_back(Pair("lavadesired", FormatDivisibleMP(bitcoinDesired)));
        ...
        for (AcceptMap::const_iterator ait = my_accepts.begin(); ait != my_accepts.end(); ++ait) {
            ...
            if (accept.getHash() == selloffer.getHash()) {
                ...
                int64_t amountToPayInBTC = calculateDesiredBTC(accept.getOfferAmountOriginal(), accept.getLAVADesiredOriginal(), amountAccepted);
...
UniValue elysium_gettransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "  \"sendingaddress\" : \"address\",     (string) the Lava address of the sender\n"
            "  \"referenceaddress\" : \"address\",   (string) a Lava address used as reference (if any)\n"
...
UniValue elysium_getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"lavacoreversion\" : \"x.x.x\",        (string) Lava Core version\n"
...
UniValue elysium_getsto(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "  \"recipients\": [                 (array of JSON objects) a list of recipients\n"
            "    {\n"
            "      \"address\" : \"address\",          (string) the Lava address of the recipient\n"
