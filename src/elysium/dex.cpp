 ...
 *
 * @see:
 * https://github.com/mastercoin-MSC/elysium/blob/mscore-0.0.9/src/elysium_dex.cpp#L439-L449
 */
...
int DEx_acceptCreate(const std::string& addressBuyer, const std::string& addressSeller, uint32_t propertyId, int64_t amountAccepted, int block, int64_t feePaid, uint64_t* nAmended)
{
    ...
    if (amountReserved > 0) {
        ...
        CMPAccept acceptOffer(amountReserved, block, offer.getBlockTimeLimit(), offer.getProperty(), offer.getOfferAmountOriginal(), offer.getLAVADesiredOriginal(), offer.getHash());
 ...
 *
 * @see:
 * https://github.com/mastercoin-MSC/elysium/blob/mscore-0.0.9/src/elysium_dex.cpp#L660-L668
 */
static int64_t calculateDExPurchase(const int64_t amountOffered, const int64_t amountDesired, const int64_t amountPaid)
{
    ...
    uint64_t LAVA_paid = static_cast<uint64_t>(amountPaid);

    const double LAVA_desired_original = acceptBTCDesired;
    const double offer_amount_original = acceptOfferAmount;

    double perc_X = (double) LAVA_paid / LAVA_desired_original;
...
int DEx_payment(const uint256& txid, unsigned int vout, const std::string& addressSeller, const std::string& addressBuyer, int64_t amountPaid, int block, uint64_t* nAmended)
{
    ...
    const int64_t amountDesired = p_accept->getLAVADesiredOriginal();
