...
bool ShouldConsensusHashBlock(int block) {
    ...
    if (!mapArgs.count("-elysiumshowblockconsensushash")) {
        return false;
    }

    const std::vector<std::string>& vecBlocks = mapMultiArgs["-elysiumshowblockconsensushash"];
...
std::string GenerateConsensusString(const CMPOffer& offerObj, const std::string& address)
{
    return strprintf("%s|%s|%d|%d|%d|%d|%d",
            ...
            offerObj.getLAVADesiredOriginal(), offerObj.getMinFee(), offerObj.getBlockTimeLimit());
}
