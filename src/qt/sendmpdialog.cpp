...
void SendMPDialog::updateFrom()
{
    ...
    if (currentSetFromAddress.empty()) {
        ...
        if (CheckFee(currentSetFromAddress, 16)) {
            ...
            ui->feeWarningLabel->setText("WARNING: The sending address is low on LAVA for transaction fees. Please topup the LAVA balance for the sending address to send Elysium transactions.");
