...
void OverviewPage::handleEnabledTorChanged(){
	...
	if(ui->checkboxEnabledTor->isChecked()){
        ...
        msgBox.setText("Please restart the Lava wallet to route your connection through Tor to protect your IP address. \nSyncing your wallet might be slower with TOR. \nNote that -torsetup in lava.conf will always override any changes made here.");
	}else{
        settings.setValue("fTorSetup", false);
        msgBox.setText("Please restart the Lava wallet to disable routing of your connection through Tor to protect your IP address. \nNote that -torsetup in lava.conf will always override any changes made here.");
	}
	msgBox.exec();
}
