    ...
    if (0<=populateResult) {
        ...
        switch(populateResult) {
            case MP_TX_NOT_FOUND:
                strText += "not a valid Lava or Elysium transaction.  Please check the transaction hash "
            ...
            case MP_TX_IS_NOT_ELYSIUM_PROTOCOL:
                strText += "a Lava transaction only.\n\nTip: You can use the debug console "
                           "'gettransaction' command to lookup specific Lava transactions.";
            break;

            default:
                strText += "of an unknown type.  If you are seeing this message please raise a bug report "
                           "with the transaction hash at github.com/lavaofficial/lava/issues.";
            break;
        }
