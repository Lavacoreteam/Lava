...
UniValue importprivkey(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey \"lavaprivkey\" ( \"label\" rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"lavaprivkey\"   (string, required) The private key (see dumpprivkey)\n"
    ...
    if(chain.nVersion == chain.VERSION_WITH_BIP39){
        throw JSONRPCError(RPC_WALLET_ERROR, "Importing wallets and private keys is disabled for mnemonic-enabled wallets."
                                             "To import your dump file, create a non-mnemonic wallet by setting \"usemnemonic=0\" in your lava.conf file, after backing up and removing your existing wallet.");
    }
...
UniValue importaddress(const UniValue& params, bool fHelp)
{
    ...
    if (address.IsValid()) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Lava address or script");
    }
...
UniValue dumpprivkey(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey \"lavaaddress\"\n"
            "\nReveals the private key corresponding to 'lavaaddress'.\n"
            "Then the importprivkey can be used with this output\n"
            "\nArguments:\n"
            "1. \"lavaaddress\"   (string, required) The Lava address for the private key\n"
            "\nResult:\n"
...
UniValue dumpprivkey_lava(const UniValue& params, bool fHelp)
{
    ...
    if(params.size() == 1 || !AuthorizationHelper::inst().authorize(__FUNCTION__ + params[0].get_str(), params[1].get_str()))
    {
        std::string warning =
            ...
            "Lava team members will never ask for this command's output and it is not needed for Znode setup or diagnosis!\n"
            "\n"
            " Please seek help on one of our public channels. \n"
            " Telegram: https://t.me/lavaproject \n"
            " Discord: https://discordapp.com/invite/4FjnQ2q\n"
            " Reddit: https://www.reddit.com/r/lava/\n"
            "\n"
            ;
...
UniValue dumpwallet(const UniValue& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by Lava %s\n", CLIENT_BUILD);
...
UniValue dumpwallet_lava(const UniValue& params, bool fHelp)
{
#ifndef UNSAFE_DUMPPRIVKEY
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
