...
CPubKey CWallet::GetKeyFromKeypath(uint32_t nChange, uint32_t nChild) {
    ...
    uint32_t nIndex = (regTest || testNet) ? BIP44_TEST_INDEX : BIP44_LAVA_INDEX;
    ...
    BOOST_FOREACH(const PAIRTYPE(const CWalletTx*, unsigned int)& pcoin, setCoins)
    {
        ...
        for (unsigned int n=0; n < min(nSearchInterval,(int64_t)nMaxStakeSearchInterval) && !fKernelFound && pindexPrev == pindexBestHeader; n++)
        {
            ...
            if (CheckKernel(pindexPrev, nBits, nTime - n, prevoutStake, stakeCache, &nBlockTime))
            {
                ...
                LogPrintf("CWallet::CreateCoinStake(): parsed kernel type=%d\n", whichType);
                if (whichType != TX_PUBKEY && whichType != TX_PUBKEYHASH)
                {
                    LogPrintf("CWallet::CreateCoinStake(): no support for kernel type=%d\n", whichType);
                ...
                if (whichType == TX_PUBKEYHASH) // pay to address type
                {
                    ...
                    {
                        LogPrintf("CWallet::CreateCoinStake(): failed to get key for kernel type=%d\n", whichType);
                ...
                if (whichType == TX_PUBKEY)
                {
                    ...
                    {
                        LogPrintf("CWallet::CreateCoinStake(): invalid key for kernel type=%d\n", whichType);
                        break; // keys mismatch
                    }
                ...
                LogPrintf("CWallet::CreateCoinStake(): added kernel type=%d\n", whichType);
                fKernelFound = true;
                break;
            }
        }
...
void CWallet::AvailableCoins(vector <COutput> &vCoins, bool fOnlyConfirmed, const CCoinControl *coinControl,
                             bool fIncludeZeroValue, AvailableCoinsType nCoinType) const {
    vCoins.clear();
...
bool CWallet::SelectCoins(const vector <COutput> &vAvailableCoins, const CAmount &nTargetValue,
                          ...
                          const CCoinControl *coinControl, AvailableCoinsType nCoinType) const {
...
bool CWallet::CreateTransaction(const vector <CRecipient> &vecSend, CWalletTx &wtxNew, CReserveKey &reservekey,
                                ...
                                bool sign, AvailableCoinsType nCoinType) {
...

    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            AvailableCoins(vAvailableCoins, true, coinControl, false, nCoinType);
