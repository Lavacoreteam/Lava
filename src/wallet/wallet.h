...
const uint32_t BIP44_LAVA_INDEX = 513; // https://github.com/satoshilabs/slips/blob/master/slip-0044.md#registered-coin-types
...
public:
    ...
    bool AcceptToMemoryPool(
        ...
        bool markLavaSpendTransactionSerial = true);
...
private:
     ...
     */
    bool SelectCoins(const std::vector<COutput>& vAvailableCoins, const CAmount& nTargetValue, std::set<std::pair<const CWalletTx*,unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl *coinControl = NULL, AvailableCoinsType nCoinType = ALL_COINS) const;
...
public:
     ...
     */
    void AvailableCoins(std::vector<COutput>& vCoins, bool fOnlyConfirmed=true, const CCoinControl *coinControl = NULL, bool fIncludeZeroValue=false, AvailableCoinsType nCoinType=ALL_COINS) const;
    ...
    /** \brief Selects coins to spend, and coins to re-mint based on the required amount to spend, provided by the user. As the lower denomination now is 0.1 lava, user's request will be rounded up to the nearest 0.1. This difference between the user's requested value, and the actually spent value will be left to the miners as a fee.
     ...
     * \returns true, if it was possible to spend exactly required(rounded up to 0.1 lava) amount using coins we have.
     */
    ...
    // If dontSpendSigma is set, spends only old lava mints if any. Used in old unit tests.
