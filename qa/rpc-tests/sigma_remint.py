...
class RemintSigmaTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        lava_mint = self.nodes[0].listunspentmintzerocoins()

        assert len(lava_mint) == 10, 'Should be 10 lava mints after lava mint, but was: {}' \
                .format(len(lava_mint))
        ...
        total_amount_to_spend = -1* len(lava_denoms)*0.05*2
