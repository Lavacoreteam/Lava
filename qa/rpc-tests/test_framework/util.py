...
def initialize_datadir(dirname, n):
    ...
    with open(os.path.join(datadir, "lava.conf"), 'w', encoding='utf8') as f:
        f.write("regtest=1\n")
...
def wait_for_bitcoind_start(process, url, i):
    '''
    Wait for lavad to start. This means that RPC is accessible and fully initialized.
    Raise an exception if lavad exits during initialization.
    '''
    while True:
        if process.poll() is not None:
            raise Exception('lavad exited with status %i during initialization' % process.returncode)
        try:
...
def initialize_chain(test_dir, num_nodes):
    ...
    if create_cache:
        ...
        for i in range(MAX_NODES):
            ...
            args = [ os.getenv("ZCOIND", "lavad"), "-server", "-keypool=1", "-datadir="+datadir, "-discover=0" ]
            if i > 0:
...
def start_node(i, dirname, extra_args=None, rpchost=None, timewait=None, binary=None):
    ...
    bitcoind_processes[i] = subprocess.Popen(args)
    if os.getenv("PYTHON_DEBUG", ""):
        print("start_node: lavad started, waiting for RPC to come up")
