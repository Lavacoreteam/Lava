...
class SigmaRemintLockedWalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        lava_mints = self.nodes[0].listunspentmintzerocoins()

        assert len(lava_mints) == 10, 'Should be 10 lava mints after lava mint, but was: {}' \
            .format(len(lava_mints))
        ...
        assert_raises(JSONRPCException, self.nodes[0].remintzerocointosigma, lava_denoms[0])
        ...
        # remint should work
        self.nodes[0].remintzerocointosigma(lava_denoms[0])
        ...
        assert len(sigma_mint) == len(lava_denoms)+1, \
        ...
        assert len(sigma_mint) == len(lava_denoms) + 2, \
