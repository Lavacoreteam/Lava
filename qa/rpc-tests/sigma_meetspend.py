...
def activate_sigma_spend(denoms, spendsize, exp_spends, exp_remints, lavad):
    for denom in denoms:
        count, size = denom
        for i in range(count):
            lavad.nodes[0].mint(size)
            lavad.nodes[0].generate(6)

    myaddr = lavad.nodes[0].listreceivedbyaddress(0, True)[0]['address']
    args = {myaddr: spendsize}
    txid = lavad.nodes[0].spendmany("", args)
    lavad.nodes[0].generate(2)

    # Should be checked spends
    spends = lavad.nodes[0].listsigmaspends(0)
